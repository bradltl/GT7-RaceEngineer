...
// xorKeyStream applies the Salsa20 stream cipher to in using the provided
// 32-byte key and 8-byte nonce. GT7 uses the 8-byte nonce form.
...
func salsaCore(out *[64]byte, in *[16]byte, k *[32]byte, c *[16]byte) {
...
func genericXORKeyStream(out, in []byte, counter *[16]byte, key *[32]byte) {
	...
	for len(in) >= 64 {
		salsaCore(&block, &counterCopy, key, &salsaSigma)
		for i, x := range block {
			out[i] = in[i] ^ x
		}
		u := uint32(1)
